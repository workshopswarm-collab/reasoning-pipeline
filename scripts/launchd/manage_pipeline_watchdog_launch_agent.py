...
WATCHDOG_SCRIPT = SCRIPTS_DIR / 'watch_pipeline.py'
RUNTIME_STATE_DIR = SCRIPTS_DIR / '.runtime-state' / 'launchd'
DEFAULT_LABEL = 'ai.openclaw.orchestrator.pipeline-watchdog'
...
def parse_args() -> argparse.Namespace:
    ...
    def add_common_render_flags(cmd: argparse.ArgumentParser) -> None:
        ...
        cmd.add_argument('--lock-file', default=str(SCRIPTS_DIR / '.runtime-state' / 'pipeline-watchdog.lock'))
        cmd.add_argument('--control-file', default=str(DEFAULT_CONTROL_FILE), help='Persisted automation control file consumed by --control-managed')
        cmd.add_argument('--apply', action='store_true', help='Render watchdog with --apply (not recommended for initial deployment)')
        cmd.add_argument('--allow-resume-swarm', action='store_true')
        cmd.add_argument('--allow-launch-synthesis', action='store_true')
        cmd.add_argument('--allow-launch-decision', action='store_true')
        cmd.add_argument('--allow-finalize-decision', action='store_true')
        cmd.add_argument('--allow-finalize-pipeline', action='store_true')
...
def watchdog_program_arguments(args: argparse.Namespace) -> list[str]:
    program = [
    ...
    if args.apply:
        program.append('--apply')
    if args.allow_resume_swarm:
        program.append('--allow-resume-swarm')
    if args.allow_launch_synthesis:
        program.append('--allow-launch-synthesis')
    if args.allow_launch_decision:
        program.append('--allow-launch-decision')
    if args.allow_finalize_decision:
        program.append('--allow-finalize-decision')
    if args.allow_finalize_pipeline:
        program.append('--allow-finalize-pipeline')
    return program
