---
type: domain_overview
domain: economics
subdomain: rates
topic: rates research overview
date_created: 2026-03-25
agent: orchestrator
certainty: medium-high
importance: high
upstream_inputs:
  - qualitative-db/40-research/source-notes/by-domain/economics/2026-03-25-interest-rate-basics.md
downstream_uses: []
related_entities: []
related_drivers: []
tags: [domain/economics, subdomain/rates, overview]
---

# Overview summary

## Why this subdomain matters

Rates research should focus on policy rates, real-vs-nominal dynamics, transmission into credit and valuation, and how rate expectations reshape macro narratives.

## Core conclusions

- Rates are one of the strongest cross-asset and cross-domain macro levers.
- Expectations about rates can matter as much as current levels.