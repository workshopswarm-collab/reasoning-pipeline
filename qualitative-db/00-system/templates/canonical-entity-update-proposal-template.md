---
type: canonical_entity_update_proposal
target:
target_path:
proposal_type: body
section:
proposed_by:
date_created:
materiality: low
conflict_level: none
confidence:
durability:
upstream_inputs: []
tags: []
---

# Canonical entity update proposal

Use this template only for proposed updates to a canonical entity note in `qualitative-db/20-entities/`.

This is a proposal artifact, not a canonical rewrite.

## Why this update is being proposed

## What current canonical claim or linkage appears outdated or misleading

## Proposal type

State whether this is primarily:
- body content
- linkage metadata
- both

## Evidence for update

- 

## Repeated conflict check

- Independent sources involved:
- Repeated agent findings involved:
- Why this is not just transient noise:

## Proposed replacement language or linkage change

## Why this meets the canonical-update threshold

## If rejected, where this should live instead
