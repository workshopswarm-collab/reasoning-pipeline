...
owner: orchestrator
tags: [system/policy, qualitative-db/10-domains, subdomains, domain-overviews]
---

# Subdomain overview policy

This policy governs when the vault should create, keep, rewrite, merge, archive, or avoid subdomain folders and `00-overview.md` files inside `qualitative-db/10-domains/`.

It exists to prevent `10-domains/` from turning into a large set of thin, repetitive retrieval anchors.

## Core rule

Top-level domain overviews in `qualitative-db/10-domains/<domain>/00-overview.md` are domain notes, not subdomain notes, and should normally leave `subdomain:` blank.
...
- `qualitative-db/60-uncategorized/archive/10-domains-subdomain-stubs/`
