...
def refresh_lane_summary(status: dict[str, Any]) -> dict[str, Any]:
    topic_id = status.get('refresh_lane_topic_id') or status.get('refresh_target_topic_id') or ''
    topic_title = status.get('refresh_lane_topic_title') or status.get('refresh_target_topic_title') or ''
    session_key = status.get('refresh_lane_session_key') or status.get('refresh_target_session_key') or ''
...
def update_terminal_summary(status: dict[str, Any]) -> None:
    ...
    status['terminal_summary'] = {
        ...
        'refresh_lane': refresh_lane_summary(status),
        'refresh_manifest_path': status.get('refresh_manifest_path', ''),
...
def write_status_file(path: Path, status: dict[str, Any]) -> None:
    ...
    status['refresh_lane'] = refresh_lane_summary(status)
