...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Create or reuse a dedicated Telegram decision lane for a decision-stage status file")
...
def main() -> None:
    ...
    with locked_status(status_path) as status:
        existing_topic_id = status.get("decision_lane_topic_id") or ""
        existing_session_key = status.get("decision_lane_session_key") or ""
        if existing_topic_id and existing_session_key:
            append_stage_event(
                status,
                stage="decision_lane_bootstrap",
                state="reused",
                message="Reused existing decision Telegram lane",
            ...
            print(json.dumps({
                ...
                "topic_title": status.get("decision_lane_topic_title", ""),
            }, indent=2 if args.pretty else None))
            return

        chat_id = str(status.get("decision_target_chat_id") or "").strip()
        if not chat_id:
            set_overall_status(status, status.get("status", "decision_ready_for_receipt"), stage="decision_lane_bootstrap", message="Missing decision_target_chat_id; cannot create dedicated decision lane")
            raise SystemExit("missing decision_target_chat_id")

        case_key = str(status.get("case_key") or "case").strip()
        dispatch_id = str(status.get("dispatch_id") or "dispatch").strip()
        title = f"{case_key} | decision | {dispatch_id[-8:]}"
    ...
    with locked_status(status_path) as status:
        if status.get("decision_lane_topic_id") and status.get("decision_lane_session_key"):
            append_stage_event(
                status,
                stage="decision_lane_bootstrap",
                state="reused",
                message="Another process already created the decision Telegram lane",
                extra={
        ...
        status.update({
            "decision_lane_topic_id": topic_id,
            "decision_lane_topic_title": topic_title,
            "decision_lane_session_key": session_key,
            "decision_target_topic_id": topic_id,
            "decision_target_topic_title": topic_title,
            "decision_target_session_key": session_key,
        })
        append_stage_event(
            status,
            stage="decision_lane_bootstrap",
            state="created",
            message="Created dedicated Telegram decision lane",
