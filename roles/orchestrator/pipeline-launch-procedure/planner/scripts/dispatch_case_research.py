...
def analysis_root(case_key: str, created_at: str, dispatch_id: str) -> Path:
    return case_root(case_key) / "analyses" / analysis_date(created_at) / dispatch_id
...
def source_note_directory(case_key: str) -> str:
    return str(case_root(case_key) / "source-notes")
...
def legacy_persona_note_path(case_key: str, slug, persona: str) -> str:
    safe_slug = slug or "market"
    return f"qualitative-db/40-research/agent-findings/{persona}/{case_key}-{safe_slug}.md"


def legacy_assumption_note_path(case_key: str, persona: str) -> str:
    return f"qualitative-db/40-research/assumption-notes/{case_key}-{persona}-assumptions.md"


def legacy_evidence_map_path(case_key: str, persona: str) -> str:
    return f"qualitative-db/40-research/evidence-maps/{case_key}-{persona}-evidence-map.md"
...
def current_file_path(case_key: str) -> str:
    return str(case_root(case_key) / "current.md")
...
def render_current_md(case_ctx: dict, *, created_at: str, dispatch_id: str, summary_note_path: str, persona_paths: dict[str, str], difficulty_profile: dict) -> str:
    lines = [
        ...
        "---",
        "",
        f"# Current view — {case_ctx.get('case_key')}",
        "",
    ...
    lines.extend([
        "",
        "## Notes",
        "- This file is a generated latest-view surface for the case.",
        "- Canonical per-analysis history lives under `analyses/<YYYY-MM-DD>/<dispatch-id>/...`.",
        "",
    ])
    return "\n".join(lines).rstrip() + "\n"
...
def compatibility_note_body(*, case_key: str, dispatch_id: str, canonical_path: str, note_kind: str, persona: str | None = None) -> str:
    vault_target = canonical_path.replace('qualitative-db/', '').removesuffix('.md')
    title_suffix = f" — {persona}" if persona else ""
    lines = [
        "---",
        ...
        (f"persona: {persona}" if persona else None),
        f"canonical_path: {canonical_path}",
        "generated_by: orchestrator",
        "---",
        "",
        f"# Latest view{title_suffix}",
        "",
...
def maybe_write_compatibility_note(path: Path, *, body: str, dry_run: bool) -> str:
    if dry_run:
        return 'dry_run'
    if path.exists():
        try:
...
def write_case_analysis_scaffolds(case_ctx: dict, *, created_at: str, dispatch_id: str, persona_paths: dict[str, str], assumption_paths: dict[str, str], evidence_paths: dict[str, str], difficulty_profile: dict, dry_run: bool) -> dict:
    ...
    legacy_persona_paths = {
        persona: legacy_persona_note_path(case_ctx['case_key'], case_ctx.get('slug'), persona)
        for persona in persona_paths
    }
    legacy_assumption_paths = {
        persona: legacy_assumption_note_path(case_ctx['case_key'], persona)
        for persona in assumption_paths
    }
    legacy_evidence_paths = {
        persona: legacy_evidence_map_path(case_ctx['case_key'], persona)
        for persona in evidence_paths
    }
    scaffolds = {
        ...
        'legacy_latest_views': {
            'personas': legacy_persona_paths,
            'assumptions': legacy_assumption_paths,
            'evidence': legacy_evidence_paths,
        },
    }
    if dry_run:
    ...
    compatibility_results = {'personas': {}, 'assumptions': {}, 'evidence': {}}
    for persona, canonical in persona_paths.items():
        compatibility_results['personas'][persona] = maybe_write_compatibility_note(
            Path(legacy_persona_paths[persona]),
            body=compatibility_note_body(
                case_key=case_ctx['case_key'],
                dispatch_id=dispatch_id,
                canonical_path=canonical,
                note_kind='persona_finding',
                persona=persona,
            ),
            dry_run=dry_run,
        )
        compatibility_results['assumptions'][persona] = 'pending_canonical_generation'
        compatibility_results['evidence'][persona] = 'pending_canonical_generation'

    scaffolds['compatibility_results'] = compatibility_results
