...
def build_topic_handoff_message(*, research_run_id: str, persona: str, case_key: str, topic_title: str, controller_topic_title: str, market_title: str, workspace_note_path: str, prompt_text: str, start_marker: str, finish_marker: str) -> str:
    return "\n".join(
        [
            ...
            "- progress: milestone-based only, or roughly once every 10 minutes max while still active",
            f"- finish: {finish_marker}",
            f"- after finishing, mark `research_run_id={research_run_id}` completed with the runtime DB helper (or failed if blocked)",
            "",
            "BEGIN_PROMPT",
            prompt_text,
            "END_PROMPT",
