...
AUTOMATION_SCRIPTS_DIR = BASE_DIR.parents[5] / 'scripts'
for path in [SCRIPTS_DIR, SUBAGENT_DIR, AUTOMATION_SCRIPTS_DIR]:
    if str(path) not in sys.path:
        sys.path.insert(0, str(path))

from automation_runtime_support import DEFAULT_SUBPROCESS_TIMEOUT_SECONDS, exclusive_lock, load_json_dict, run_subprocess  # noqa: E402
...
def process_running(pid: Any) -> bool:
    try:
        ...
        return False
    proc = run_subprocess(["ps", "-p", str(pid_int), "-o", "pid="], timeout_seconds=DEFAULT_SUBPROCESS_TIMEOUT_SECONDS)
...
def validate_status_payload(status_path: Path) -> dict[str, Any]:
    status = load_json_dict(status_path, artifact_name='synthesis status file', required_keys=['bundle_path'])
    requests = status.get('reasoning_sidecar_requests')
    if requests is not None and not isinstance(requests, list):
        raise RuntimeError(f'synthesis status reasoning_sidecar_requests must be a list: {status_path}')
    return status
...
def main() -> int:
    ...
    with exclusive_lock(
        ...
        error_message=f'another synthesis repair already holds {status_path}',
    ):
        status = validate_status_payload(status_path)
        bundle_path = WORKSPACE_ROOT / status["bundle_path"]
        bundle = load_json_dict(bundle_path, artifact_name='synthesis bundle')
