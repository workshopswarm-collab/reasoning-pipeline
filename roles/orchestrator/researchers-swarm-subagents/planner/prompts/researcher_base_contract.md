...
2. **Use assigned paths exactly**
   ...
   - treat unresolved canonical mapping as an explicit output obligation: if a causally or structurally important entity/driver lacks a clean canonical match, record it in `proposed_entities` or `proposed_drivers` rather than forcing a weak canonical fit
...
4. **Preserve provenance without drifting**
   ...
   - retrieval minimums refer to search effort and coverage, not guaranteed hit count; never invent markets, analogs, events, or other retrieval artifacts to satisfy expected QMD volume
   - if retrieval is sparse or returns no relevant matches, report that explicitly, broaden/reformulate search as appropriate, and reduce confidence accordingly
   - only provenance-backed items from actual retrieval output, approved memory, or explicitly cited external sources may be used or counted
