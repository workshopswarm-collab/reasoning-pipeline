...
from lib.proposed_causal_shadow_outcomes import load_case_artifacts  # noqa: E402
from lib.proposed_causal_trial_outcomes import judge_trial_row  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Deterministically score proposed-causal trial exposures as helpful / neutral / harmful / unclear')
    ...
    parser.add_argument('--include-preview-only', action='store_true')
...
def matches_filters(row: dict[str, Any], args: argparse.Namespace) -> bool:
    ...
    if args.only_unjudged and str(row.get('outcome_label') or '').strip():
        return False
    if not args.include_preview_only and not bool(row.get('injected')):
        return False
    return True
...
def main() -> int:
    ...
    trial_present = table_exists('proposed_causal_trial_exposures', db_url=resolved_db_url, psql_bin=args.psql) if resolved_db_url else False
    ...
    if resolved_db_url and trial_present:
        trial_missing = missing_columns('proposed_causal_trial_exposures', REQUIRED_TRIAL_COLUMNS, db_url=resolved_db_url, psql_bin=args.psql)
        if trial_missing:
            trial_present = False

    trial_rows = exec_sql(args.psql, resolved_db_url, TRIAL_ROWS_SQL, {}) if resolved_db_url and trial_present else []
    ...
    for row in occurrence_rows:
        key = (str(row.get('case_key') or '').strip(), str(row.get('proposal_id') or '').strip())
    ...
    skipped_preview_only = 0

    for row in trial_rows:
        if not isinstance(row, dict):
            continue
        if not args.include_preview_only and not bool(row.get('injected')):
            skipped_preview_only += 1
            continue
        ...
        judgment = judge_trial_row(
            row,
            occurrence_rows=occurrence_by_case_and_proposal.get((case_key, proposal_id), []),
        ...
        if resolved_db_url and trial_present and not args.dry_run:
    ...
    output: dict[str, Any] = {
        ...
        'skipped_preview_only_count': skipped_preview_only,
        'table_present': {
            'proposed_causal_trial_exposures': trial_present,
    ...
    if not resolved_db_url:
        ...
        output['missing_columns'] = {'proposed_causal_trial_exposures': trial_missing}
    elif not trial_present:
        output['warning'] = 'proposed_causal_trial_exposures table missing; apply roles/evaluator/sql/036_proposed_causal_trial_exposures.sql and roles/evaluator/sql/037_proposed_causal_trial_outcome_labels.sql'
